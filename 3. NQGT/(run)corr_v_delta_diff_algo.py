...
def run_GAMP_v_others(p, n_list, alpha, nu, lamb, bound, num_iter, num_runs, num_MC_samples):

  num_deltas = len(n_list)

  mean_final_corr_list_AMP = np.zeros(num_deltas)
  mean_final_corr_list_BPD = np.zeros(num_deltas)
  var_final_corr_list_AMP = np.zeros((num_runs, num_deltas))
  var_final_corr_list_BPD = np.zeros((num_runs, num_deltas))
  ...
  for n_index in range(len(n_list)):
    ...
    for run_num in range(num_runs):
      ...
      beta_hat_BPDN = run_BPDN(n, p, X, y, bound)
      ...
      # BPD
      corr_emp = norm_sq_corr(beta, beta_hat_BPDN)
      mean_final_corr_list_BPD[n_index] += corr_emp
      var_final_corr_list_BPD[run_num][n_index] = corr_emp

  mean_final_corr_list_AMP = mean_final_corr_list_AMP / num_runs
  mean_final_corr_list_BPD = mean_final_corr_list_BPD / num_runs

  print('mean_final_corr_list_AMP\n',mean_final_corr_list_AMP)
  print('mean_final_corr_list_LP\n',mean_final_corr_list_BPD)

  SD_final_corr_list_AMP = np.sqrt(np.sum(np.square(var_final_corr_list_AMP - mean_final_corr_list_AMP), axis=0) / num_runs)
  SD_final_corr_list_BPD = np.sqrt(np.sum(np.square(var_final_corr_list_BPD - mean_final_corr_list_BPD), axis=0) / num_runs)

  print('SD_final_corr_list_AMP\n',SD_final_corr_list_AMP)
  print('SD_final_corr_list_LP\n',SD_final_corr_list_BPD)

  return [mean_final_corr_list_AMP, mean_final_corr_list_BPD, SD_final_corr_list_AMP, SD_final_corr_list_BPD]
...
output_list = run_GAMP_v_others(p, n_list, alpha, nu, lamb, bound, num_iter, num_runs, num_MC_samples)
