...
def run_GAMP(n, p, X, y, alpha, nu, lamb, beta, beta_bar_mean, beta_bar_var, beta_hat_0, num_iter):
...
def run_GAMP_v_SE_multi_delta(p, n_list, alpha, nu, lamb, num_iter, num_runs, num_MC_samples):
  
  num_deltas = len(n_list)

  mean_final_corr_list = np.zeros(num_deltas)
  var_final_corr_list = np.zeros((num_runs, num_deltas))

  mean_final_corr_list_SE = np.zeros(num_deltas)
  var_final_corr_list_SE = np.zeros((num_runs, num_deltas))
  ...
  for n_index in range(len(n_list)):
    ...
    for run_num in range(num_runs):
      ...
      beta_hat_storage, mu_k_b_storage = run_GAMP(n, p, X_tilde, y_tilde, alpha, nu, lamb, beta, beta_bar_mean, beta_bar_var, beta_hat_0, num_iter)

      # GAMP
      beta_hat = beta_hat_storage[-1]
      corr_emp = norm_sq_corr(beta, beta_hat)
      mean_final_corr_list[n_index] += corr_emp
      var_final_corr_list[run_num][n_index] = corr_emp

      # State evolution
      mu_k_b = mu_k_b_storage[-1]
      sigma_k_b = np.sqrt(mu_k_b)
      corr_theo = norm_sq_corr_SE(mu_k_b, sigma_k_b, num_MC_samples, nu)
      mean_final_corr_list_SE[n_index] += corr_theo
      var_final_corr_list_SE[run_num][n_index] = corr_theo

  mean_final_corr_list = mean_final_corr_list / num_runs
  mean_final_corr_list_SE = mean_final_corr_list_SE / num_runs

  print('mean_final_corr_list\n',mean_final_corr_list)
  print('mean_final_corr_list_SE\n',mean_final_corr_list_SE)

  SD_final_corr_list = np.sqrt(np.sum(np.square(var_final_corr_list - mean_final_corr_list), axis=0) / num_runs)
  SD_final_corr_list_SE = np.sqrt(np.sum(np.square(var_final_corr_list_SE - mean_final_corr_list_SE), axis=0) / num_runs)

  print('SD_final_corr_list\n',SD_final_corr_list)
  print('SD_final_corr_list_SE\n',SD_final_corr_list_SE)

  return [mean_final_corr_list, mean_final_corr_list_SE, SD_final_corr_list, SD_final_corr_list_SE]

p = 500
n_list = [int(0.1*p), int(0.2*p), int(0.3*p), int(0.4*p), int(0.5*p), int(0.6*p), int(0.7*p), int(0.8*p), int(0.9*p), p]
alpha = 0.5
nu = 0.1
lamb1 = 0.1
lamb2 = 0.2
...
output_list1 = run_GAMP_v_SE_multi_delta(p, n_list, alpha, nu, lamb1, num_iter, num_runs, num_MC_samples)
output_list2 = run_GAMP_v_SE_multi_delta(p, n_list, alpha, nu, lamb2, num_iter, num_runs, num_MC_samples)
output_list3 = run_GAMP_v_SE_multi_delta(p, n_list, alpha, nu, lamb3, num_iter, num_runs, num_MC_samples)
output_list = [output_list1, output_list2, output_list3]
save('corr_v_delta', np.array(output_list))