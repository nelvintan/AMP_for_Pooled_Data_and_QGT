...
def run_BPDN(n, p, X, y, bound):
  ...
  # constraints.append(cp.sum_squares(y - X @ beta_hat) <= bound)
...
def run_BPDN_threshold(n, p, X, y, bound, threshold):

  beta_hat = run_BPDN(n, p, X, y, bound)

  return quantize(beta_hat, threshold)

def run_FPR_v_FNR(p, n, threshold_list, alpha, nu, lamb, bound, num_iter, num_runs, num_MC_samples):
  ...
  mean_final_FPR_list_BPD = np.zeros(num_thresholds)
  mean_final_FNR_list_BPD = np.zeros(num_thresholds)
  ...
  for threshold_index in range(num_thresholds):
    ...
    for run_num in range(num_runs):
      ...
      beta_hat_BPD = run_BPDN_threshold(n, p, X, y, bound, threshold)
      ...
      # LP
      FPR = (np.sum(beta_hat_BPD) - np.inner(beta, beta_hat_BPD)) / num_nondef
      FNR = ((p - np.inner(beta, beta_hat_BPD)) - num_nondef) / num_def
      mean_final_FPR_list_BPD[threshold_index] += FPR
      mean_final_FNR_list_BPD[threshold_index] += FNR
  ...
  mean_final_FPR_list_BPD = mean_final_FPR_list_BPD / num_runs
  mean_final_FNR_list_BPD = mean_final_FNR_list_BPD / num_runs
  ...
  print('mean_final_FPR_list_BPD\n', mean_final_FPR_list_BPD)
  print('mean_final_FNR_list_BPD\n', mean_final_FNR_list_BPD)

  return [mean_final_FPR_list, mean_final_FNR_list, mean_final_FPR_SE_list, mean_final_FNR_SE_list, mean_final_FPR_list_BPD, mean_final_FNR_list_BPD]
...
bound = lamb*np.sqrt(p) # for l-infinity norm in constrain

output_list = run_FPR_v_FNR(p, n, threshold_list, alpha, nu, lamb, bound, num_iter, num_runs, num_MC_samples)
