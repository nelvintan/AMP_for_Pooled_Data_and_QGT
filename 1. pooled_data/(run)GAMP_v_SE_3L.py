...
def E_Z_given_Zk_Ybar_L3(Z_k, Y_bar):
  return Y_bar

def g_k_bayes_L3(Z_k, Y_bar, Sigma_k):

  # mat1 = Var_Z_given_Zk_L3(Sigma_k)
  mat1 = my_round(Var_Z_given_Zk_L3(Sigma_k))
  vec2 = E_Z_given_Zk_Ybar_L3(Z_k, Y_bar)
...
def g_k_bayes_wrapper_L3(Z_k_and_Y_bar, Sigma_k):
  Z_k = Z_k_and_Y_bar[:3]
  Y_bar = Z_k_and_Y_bar[3:]
  return g_k_bayes_L3(Z_k, Y_bar, Sigma_k)
...
def run_matrix_GAMP_L3(n, p, X, B_bar_prob, B, B_bar_mean, B_bar_cov, B_hat_0, B_hat_0_row_mean, B_hat_0_row_cov, num_iter):
  ...
  for k in range(num_iter):
    ...
    try:
      R_hat_k = np.apply_along_axis(g_k_bayes_wrapper_L3, 1, Theta_k_and_Y, Sigma_k)
    except:
...
def run_GAMP_v_SE_multi_delta(alpha, p, n_list, B_bar_prob_vec, B_bar_mean_vec, B_bar_cov_vec, num_iter, num_runs, num_MC_samples):
  ...
  for n_index in range(len(n_list)):
    ...
    for run_num in range(num_runs):
      ...
      # For B_bar_prob1
      indices = np.random.choice(np.array([0, 1, 2]), size=p, p=B_bar_prob1)
      ...
      B_hat_storage1, M_k_B_storage1 = run_matrix_GAMP_L3(n, p, X_tilde, B_bar_prob1, B1, B_bar_mean1, B_bar_cov1, B_hat_0_1, B_hat_0_row_mean1, B_hat_0_row_cov1, num_iter)
      num_iter_ran1 = len(B_hat_storage1)

      # For B_bar_prob2
      indices = np.random.choice(np.array([0, 1, 2]), size=p, p=B_bar_prob2)
      ...
      B_hat_storage2, M_k_B_storage2 = run_matrix_GAMP_L3(n, p, X_tilde, B_bar_prob2, B2, B_bar_mean2, B_bar_cov2, B_hat_0_2, B_hat_0_row_mean2, B_hat_0_row_cov2, num_iter)
      ...
      corr_SE = SE_corr_L3(M_k_B, B_bar_prob1, num_MC_samples)
      ...
      corr_SE = SE_corr_L3(M_k_B, B_bar_prob2, num_MC_samples)
  ...
  print('SD_final_MSE_list1\n', SD_final_corr_list1)
  print('SD_final_MSE_list_SE1\n', SD_final_corr_list_SE1)

  print('SD_final_MSE_list2\n', SD_final_corr_list2)
  print('SD_final_MSE_list_SE2\n', SD_final_corr_list_SE2)
...
alpha = 0.5
B_bar_prob1 = [1/3, 1/3, 1/3]
B_bar_prob2 = [0.1, 0.3, 0.6]
B_bar_prob_vec = [B_bar_prob1, B_bar_prob2]
B_bar_mean1 = B_bar_prob1
B_bar_mean2 = B_bar_prob2
B_bar_mean_vec = [B_bar_mean1, B_bar_mean2]
B_bar_cov1 = np.array([
                    [B_bar_prob1[0]-(B_bar_prob1[0])**2, -1*B_bar_prob1[0]*B_bar_prob1[1], -1*B_bar_prob1[0]*B_bar_prob1[2]],
                    [-1*B_bar_prob1[1]*B_bar_prob1[0], B_bar_prob1[1]-(B_bar_prob1[1])**2, -1*B_bar_prob1[1]*B_bar_prob1[2]],
                    [-1*B_bar_prob1[2]*B_bar_prob1[0], -1*B_bar_prob1[2]*B_bar_prob1[1], B_bar_prob1[2]-(B_bar_prob1[2])**2]
                    ])
B_bar_cov2 = np.array([
                    [B_bar_prob2[0]-(B_bar_prob2[0])**2, -1*B_bar_prob2[0]*B_bar_prob2[1], -1*B_bar_prob2[0]*B_bar_prob2[2]],
                    [-1*B_bar_prob2[1]*B_bar_prob2[0], B_bar_prob2[1]-(B_bar_prob2[1])**2, -1*B_bar_prob2[1]*B_bar_prob2[2]],
                    [-1*B_bar_prob2[2]*B_bar_prob2[0], -1*B_bar_prob2[2]*B_bar_prob2[1], B_bar_prob2[2]-(B_bar_prob2[2])**2]
                    ])
B_bar_cov_vec = [B_bar_cov1, B_bar_cov2]
