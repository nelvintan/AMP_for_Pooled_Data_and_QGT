...
def E_Z_given_Zk_Ybar(Z_k, Y_bar, Sigma_k, Psi_cov):
  cov_matrix = np.zeros((6, 6))
  cov_matrix[:4, :4] = Sigma_k
  cov_matrix[4:, :2] = Sigma_k[:2, :2]
  cov_matrix[:2, 4:] = Sigma_k[:2, :2]
  cov_matrix[4:, 2:4] = Sigma_k[:2, 2:]
  cov_matrix[2:4, 4:] = Sigma_k[:2, 2:]
  cov_matrix[4:, 4:] = Sigma_k[:2, :2] + Psi_cov
  output = np.dot(cov_matrix[:2, 2:], np.dot(linalg.pinv(cov_matrix[2:, 2:]), np.concatenate((Z_k, Y_bar), axis=0)))
  return output

def g_k_bayes(Z_k, Y_bar, Sigma_k, Psi_cov):

  # mat1 = Var_Z_given_Zk(Sigma_k)
  mat1 = my_round(Var_Z_given_Zk(Sigma_k))
  vec2 = E_Z_given_Zk_Ybar(Z_k, Y_bar, Sigma_k, Psi_cov)
...
def g_k_bayes_wrapper(Z_k_and_Y_bar, Sigma_k, Psi_cov):
  Z_k = Z_k_and_Y_bar[:2]
  Y_bar = Z_k_and_Y_bar[2:]
  return g_k_bayes(Z_k, Y_bar, Sigma_k, Psi_cov)
...
def run_matrix_GAMP(n, p, X, Y, Psi_cov, B_bar_prob, B, B_bar_mean, B_bar_cov, B_hat_0, B_hat_0_row_mean, B_hat_0_row_cov, num_iter):
  ...
  for k in range(num_iter):
    ...
    R_hat_k = np.apply_along_axis(g_k_bayes_wrapper, 1, Theta_k_and_Y, Sigma_k, Psi_cov)
...
def run_GAMP_v_SE_multi_delta(alpha, p, sigma_vec, n_list, B_bar_prob, B_bar_mean, B_bar_cov, num_iter, num_runs, num_MC_samples):
  ...
  for n_index in range(len(n_list)):
    ...
    for run_num in range(num_runs):
      ...
      # sigma = 0.1
      tilde_Psi_cov = (1 / (delta * alpha * (1 - alpha))) * np.diag(np.array([sigma1**2, sigma1**2]))
      noise_matrix = normal(0, np.sqrt((1 / (delta * alpha * (1 - alpha))) * sigma1**2), (n, 2)) 
      Y_tilde = np.dot(X_tilde, B) + noise_matrix
      B_hat_storage1, M_k_B_storage1 = run_matrix_GAMP(n, p, X_tilde, Y_tilde, tilde_Psi_cov, B_bar_prob, B, B_bar_mean, B_bar_cov, B_hat_0, B_hat_0_row_mean, B_hat_0_row_cov, num_iter)
      num_iter_ran1 = len(B_hat_storage1)

      # sigma = 0.3
      tilde_Psi_cov = (1 / (delta * alpha * (1 - alpha))) * np.diag(np.array([sigma2**2, sigma2**2]))
      noise_matrix = normal(0, np.sqrt((1 / (delta * alpha * (1 - alpha))) * sigma2**2), (n, 2)) 
      Y_tilde = np.dot(X_tilde, B) + noise_matrix
      B_hat_storage2, M_k_B_storage2 = run_matrix_GAMP(n, p, X_tilde, Y_tilde, tilde_Psi_cov, B_bar_prob, B, B_bar_mean, B_bar_cov, B_hat_0, B_hat_0_row_mean, B_hat_0_row_cov, num_iter)
      num_iter_ran2 = len(B_hat_storage2)

      # sigma = 0.5
      tilde_Psi_cov = (1 / (delta * alpha * (1 - alpha))) * np.diag(np.array([sigma3**2, sigma3**2]))
      noise_matrix = normal(0, np.sqrt((1 / (delta * alpha * (1 - alpha))) * sigma3**2), (n, 2)) 
      Y_tilde = np.dot(X_tilde, B) + noise_matrix
      B_hat_storage3, M_k_B_storage3 = run_matrix_GAMP(n, p, X_tilde, Y_tilde, tilde_Psi_cov, B_bar_prob, B, B_bar_mean, B_bar_cov, B_hat_0, B_hat_0_row_mean, B_hat_0_row_cov, num_iter)
...
save('GAMP_v_SE_noisy_2L', np.array(output_list))