...
def SE_norm_sq_corr(M_k_B, B_bar_prob, num_MC_samples):
  ...
  E_beta1bar_sq = B_bar_prob[0]
  E_f1_beta1bar = 0
  E_f1_sq = 0
  E_beta2bar_sq = B_bar_prob[1]
  E_f2_beta2bar = 0
  E_f2_sq = 0
...
def MSE(B, B_hat):
  ...
  for j in range(p):
    B_hat_j = B_hat[j, :]
    B_j = B[j, :]
    output += linalg.norm(B_hat_j - B_j)**2
  output = output / p
  return output

def SE_MSE(M_k_B, B_bar_prob, num_MC_samples):
  ...
  for i in range(num_MC_samples):
    ...
    s = np.dot(M_k_B, B_bar_sample) + G_k_B_sample
    f = f_k_bayes(s, M_k_B, T_k_B, B_bar_prob)
    output += linalg.norm(f - B_bar_sample)**2
...
def run_matrix_GAMP(n, p, X, B_bar_prob, B, B_bar_mean, B_bar_cov, B_hat_0, B_hat_0_row_mean, B_hat_0_row_cov, num_iter):
...
def get_exp_vec(z, Sigma, d, pi):
  exponent_vec = np.zeros(d)
  for r in range(d):
    one_hot_r = one_hot(r, num_classes=d)
    exponent_vec[r] = np.log(pi[r]) + (-1/2)*np.dot(z-one_hot_r,np.dot(linalg.pinv(Sigma),z-one_hot_r))
  return exponent_vec

def eta(z, Sigma, d, pi):
  output = 0
  for r in range(d):
    one_hot_r = one_hot(r, num_classes=d)
    exponent_r = (-1/2)*np.dot(z-one_hot_r,np.dot(linalg.pinv(Sigma),z-one_hot_r))
    exponent_vec = get_exp_vec(z, Sigma, d, pi)
    softmax_term = np.exp(exponent_r - logsumexp(exponent_vec))
    output += pi[r] * one_hot_r * softmax_term
  return output
...
def run_AMP(n, m, d, pi, A, A_bar, h_bar, x, x_hat_0, num_iter):

  # Initialization
  # x_hat_0 = np.repeat(pi[None], n, axis=0)
  B_0 = np.zeros((n, d, d))
  for i in range(n):
    x_hat_0_i = x_hat_0[i]
    B_0[i] = np.diag(x_hat_0_i) - np.outer(x_hat_0_i, x_hat_0_i)
  w_minus_1 = np.zeros((m, d))
  V_minus_1 = np.zeros((m, d, d))
  for a in range(m):
    V_minus_1[a] = np.eye(d)
  
  # iterates
  x_hat_t = x_hat_0
  B_t = B_0
  w_t_minus_1 = w_minus_1
  V_t_minus_1 = V_minus_1

  # storage
  x_hat_storage = [x_hat_0,]
  ...
  for t in range(num_iter):
    ...
    for a in range(m):
      sum_term = 0
      for j in range(n):
        V_t[a] += (A_bar[a,j]**2) * B_t[j]
        sum_term += A_bar[a,j] * x_hat_t[j]
      w_t[a] = sum_term - np.dot(np.dot(V_t[a],linalg.pinv(V_t_minus_1[a])),h_bar[a]-w_t_minus_1[a])

    if (np.isnan(V_t).any() or np.isnan(w_t).any()):
      print('=== EARLY STOPPAGE (nan encountered) ===')
      break

    Sigma_t = np.zeros((n,d,d))
    z_t = np.zeros((n,d))
    x_hat_t_plus_1 = np.zeros((n,d))
    B_t_plus_1 = np.zeros((n, d, d))
    for i in range(n):
      Sigma_t_i = np.zeros((d,d))
      sum_term = np.zeros(d)
      for b in range(m):
        Sigma_t_i += (A_bar[b,i]**2) * linalg.pinv(V_t[b])
        sum_term += A_bar[b,i] * np.dot(linalg.pinv(V_t[b]),h_bar[b]-w_t[b])
      Sigma_t[i] = linalg.pinv(Sigma_t_i)
      z_t[i] = x_hat_t[i] + np.dot(Sigma_t[i],sum_term)
      x_hat_t_plus_1[i] = eta(z_t[i], Sigma_t[i], d, pi)
      B_t_plus_1[i] = np.diag(x_hat_t_plus_1[i]) - np.outer(x_hat_t_plus_1[i],x_hat_t_plus_1[i])

    if (np.isnan(Sigma_t).any() or np.isnan(z_t).any() or np.isnan(x_hat_t_plus_1).any() or np.isnan(B_t_plus_1).any()):
      print('=== EARLY STOPPAGE (nan encountered) ===')
      break

    # deciding termination of algorithm
    current_min_corr = corr(x, x_hat_t_plus_1)
    if (prev_min_corr >= current_min_corr):
      print('=== EARLY STOPPAGE (no improvement in estimate) ===')
      break
    else:
      prev_min_corr = current_min_corr

    # Updating iterates
    x_hat_storage.append(x_hat_t_plus_1)
    x_hat_t = x_hat_t_plus_1
    B_t = B_t_plus_1
    w_t_minus_1 = w_t
    V_t_minus_1 = V_t
  
  return x_hat_storage

''' Comparing AMP's '''

def run_GAMP_v_SE_multi_delta(alpha, p, n_list, B_bar_prob, B_bar_mean, B_bar_cov, B_hat_0_row_mean, B_hat_0_row_cov, num_iter, num_runs):
  ...
  for n_index in range(len(n_list)):
    ...
    for run_num in range(num_runs):
      ...
      B_hat_storage, M_k_B_storage = run_matrix_GAMP(n, p, X_tilde, B_bar_prob, B, B_bar_mean, B_bar_cov, B_hat_0, B_hat_0_row_mean, B_hat_0_row_cov, num_iter)
      ...
      m = n
      n = p
      A_bar = (A - alpha) / np.sqrt(n)
      pi = np.sum(x, axis=0) / n
      h_bar = (h - alpha*n*pi) / np.sqrt(n)
      x_hat_0 = B_hat_0

      x_hat_storage = run_AMP(n, m, d, pi, A, A_bar, h_bar, x, x_hat_0, num_iter)
      num_iter_ran = len(x_hat_storage)
      x_hat = x_hat_storage[num_iter_ran - 1]
      corr_emp = corr(x, x_hat)
      mean_final_corr_list2[m_index] += corr_emp
      var_final_corr_list2[run_num][m_index] = corr_emp
  ...
  return [mean_final_corr_list1, SD_final_corr_list1, mean_final_corr_list2, SD_final_corr_list2]
...
output_list = run_GAMP_v_SE_multi_delta(alpha, p, n_list, B_bar_prob, B_bar_mean, B_bar_cov, B_hat_0_row_mean, B_hat_0_row_cov, num_iter, num_runs)
save('GAMP_v_delta_Alaoui_0109', np.array(output_list))
...
output_list = run_GAMP_v_SE_multi_delta(alpha, p, n_list, B_bar_prob, B_bar_mean, B_bar_cov, B_hat_0_row_mean, B_hat_0_row_cov, num_iter, num_runs)
save('GAMP_v_delta_Alaoui_0505', np.array(output_list))