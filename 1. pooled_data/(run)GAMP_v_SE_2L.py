...
def run_GAMP_v_SE_multi_delta(alpha, p, n_list, B_bar_prob_vec, B_bar_mean_vec, B_bar_cov_vec, num_iter, num_runs, num_MC_samples):
  
  B_bar_prob1 = B_bar_prob_vec[0]
  B_bar_prob2 = B_bar_prob_vec[1]
  B_bar_prob3 = B_bar_prob_vec[2]
  B_bar_mean1 = B_bar_mean_vec[0]
  B_bar_mean2 = B_bar_mean_vec[1]
  B_bar_mean3 = B_bar_mean_vec[2]
  B_bar_cov1 = B_bar_cov_vec[0]
  B_bar_cov2 = B_bar_cov_vec[1]
  B_bar_cov3 = B_bar_cov_vec[2]

  B_hat_0_row_mean1 = B_bar_mean1
  B_hat_0_row_mean2 = B_bar_mean2
  B_hat_0_row_mean3 = B_bar_mean3
  B_hat_0_row_cov1 = B_bar_cov1
  B_hat_0_row_cov2 = B_bar_cov2
  B_hat_0_row_cov3 = B_bar_cov3
  ...
  for n_index in range(len(n_list)):
    ...
    for run_num in range(num_runs):
      ...
      # pi_1
      indices = np.random.choice(np.array([0, 1]), size=p, p=B_bar_prob1)
      B1 = np.eye(np.max(indices)+1)[indices]
      indices = np.random.choice(np.array([0, 1]), size=p, p=B_bar_prob1)
      B_hat_0_1 = np.eye(np.max(indices)+1)[indices]
      Y = np.dot(X, B1)
      pi_matrix = np.dot(np.ones((n, p)), B1) / p
      Y_tilde = (Y - alpha * p * pi_matrix) / np.sqrt(n * alpha * (1-alpha))
      B_hat_storage1, M_k_B_storage1 = run_matrix_GAMP(n, p, X_tilde, Y_tilde, B_bar_prob1, B1, B_bar_mean1, B_bar_cov1, B_hat_0_1, B_hat_0_row_mean1, B_hat_0_row_cov1, num_iter)
      num_iter_ran1 = len(B_hat_storage1)

      # pi_2
      indices = np.random.choice(np.array([0, 1]), size=p, p=B_bar_prob2)
      B2 = np.eye(np.max(indices)+1)[indices]
      indices = np.random.choice(np.array([0, 1]), size=p, p=B_bar_prob2)
      B_hat_0_2 = np.eye(np.max(indices)+1)[indices]
      Y = np.dot(X, B2)
      pi_matrix = np.dot(np.ones((n, p)), B2) / p
      Y_tilde = (Y - alpha * p * pi_matrix) / np.sqrt(n * alpha * (1-alpha))
      B_hat_storage2, M_k_B_storage2 = run_matrix_GAMP(n, p, X_tilde, Y_tilde, B_bar_prob2, B2, B_bar_mean2, B_bar_cov2, B_hat_0_2, B_hat_0_row_mean2, B_hat_0_row_cov2, num_iter)
      num_iter_ran2 = len(B_hat_storage2)

      # pi_3
      indices = np.random.choice(np.array([0, 1]), size=p, p=B_bar_prob3)
      B3 = np.eye(np.max(indices)+1)[indices]
      indices = np.random.choice(np.array([0, 1]), size=p, p=B_bar_prob3)
      B_hat_0_3 = np.eye(np.max(indices)+1)[indices]
      Y = np.dot(X, B3)
      pi_matrix = np.dot(np.ones((n, p)), B3) / p
      Y_tilde = (Y - alpha * p * pi_matrix) / np.sqrt(n * alpha * (1-alpha))
      B_hat_storage3, M_k_B_storage3 = run_matrix_GAMP(n, p, X_tilde, Y_tilde, B_bar_prob3, B3, B_bar_mean3, B_bar_cov3, B_hat_0_3, B_hat_0_row_mean3, B_hat_0_row_cov3, num_iter)
      num_iter_ran3 = len(B_hat_storage3)

      # GAMP
      B_hat = B_hat_storage1[num_iter_ran1 - 1]
      corr_emp = corr(B1, quantize(B_hat))
      ...
      corr_SE = SE_corr(M_k_B, B_bar_prob1, num_MC_samples)
      ...
      corr_emp = corr(B2, quantize(B_hat))
      ...
      corr_SE = SE_corr(M_k_B, B_bar_prob2, num_MC_samples)
      ...
      corr_emp = corr(B3, quantize(B_hat))
      ...
      corr_SE = SE_corr(M_k_B, B_bar_prob3, num_MC_samples)
...
B_bar_mean1 = B_bar_prob1
B_bar_cov1 = np.array([
    [B_bar_prob1[0]-(B_bar_prob1[0])**2, -1*B_bar_prob1[0]*B_bar_prob1[1]],
    [-1*B_bar_prob1[0]*B_bar_prob1[1], B_bar_prob1[1]-(B_bar_prob1[1])**2]
    ])
B_bar_mean2 = B_bar_prob2
B_bar_cov2 = np.array([
    [B_bar_prob2[0]-(B_bar_prob2[0])**2, -1*B_bar_prob2[0]*B_bar_prob2[1]],
    [-1*B_bar_prob2[0]*B_bar_prob2[1], B_bar_prob2[1]-(B_bar_prob2[1])**2]
    ])
B_bar_prob3 = [0.1, 0.9]
B_bar_mean3 = B_bar_prob3
B_bar_cov3 = np.array([
    [B_bar_prob3[0]-(B_bar_prob3[0])**2, -1*B_bar_prob3[0]*B_bar_prob3[1]],
    [-1*B_bar_prob3[0]*B_bar_prob3[1], B_bar_prob3[1]-(B_bar_prob3[1])**2]
    ])
B_bar_prob_vec = [B_bar_prob1, B_bar_prob2, B_bar_prob3]
B_bar_mean_vec = [B_bar_mean1, B_bar_mean2, B_bar_mean3]
B_bar_cov_vec = [B_bar_cov1, B_bar_cov2, B_bar_cov3]

output_list = run_GAMP_v_SE_multi_delta(alpha, p, n_list, B_bar_prob_vec, B_bar_mean_vec, B_bar_cov_vec, num_iter, num_runs, num_MC_samples)
save('output_list', np.array(output_list))